package com.veteran.presency.view.absen
...
import com.veteran.presency.BuildConfig
import com.veteran.presency.R
import com.veteran.presency.utils.BitmapManager.bitmapToBase64
import com.veteran.presency.viewmodel.AbsenViewModel
...
class AbsenActivity : AppCompatActivity() {
    ...
    private fun setUploadData() {
        btnAbsen.setOnClickListener {
            ...
            val strNIM = inputNIM.text.toString()
            val strTanggal = inputTanggal.text.toString()
            val strMatkul = inputMatkul.text.toString()
            val strStatus = inputStatus.text.toString()
            val strKeterangan = inputKeterangan.text.toString()

            if (strFilePath.equals(null)
                || strNama.isEmpty()
                || strNIM.isEmpty()
                || strCurrentLocation.isEmpty()
                || strTanggal.isEmpty()
                || strMatkul.isEmpty()
                || strStatus.isEmpty()
                || strKeterangan.isEmpty()) {
                Toast.makeText(this@AbsenActivity,
                    "Mohon lengkapi Data yang kosong!", Toast.LENGTH_SHORT).show()
            } else {
                absenViewModel.addDataAbsen(
                    ...
                    strKeterangan)
                Toast.makeText(this@AbsenActivity,
                    "Absen berhasil diinput", Toast.LENGTH_SHORT).show()
                finish()
            }
        }
    }
